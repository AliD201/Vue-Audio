    ...
    <div class="flex relative">
      ...
      <div class="float-left w-7 h-7 leading-3 ml-7 mt-2 player-scrub">
        <div
         ...
         v-if="currentAudio.modified_name"
         >
          <span class="song-title">{{ currentAudio.modified_name }}</span> by
          <span class="song-artist"> (Uploaded By {{  currentAudio.display_name }})</span>
        </div>
        <!-- Scrub Container  -->
        <span
         ...
         >
          <!-- Player Ball -->
          <span class="absolute text-gray-800 text-lg" 
          style=" top: -12px;"
...
computed: {
  ...mapGetters(['playing']),
  ...mapState(['duration', 'seek', 'playerProgress', 'currentAudio']),
},
methods: {
